...
{
	int result;
	if ((setegid((gid_t) 2002) == 0) && (seteuid((uid_t) 2002) == 0)) {
		result = do_file_op(file);
		/* expected result = OP_READ || OP_WRITE */
		report(2, OP_READ + OP_WRITE, result);
		seteuid((uid_t) 0);
...
{
	int result;
	if ((setegid((gid_t) 2003) == 0) && (seteuid((uid_t) 2003) == 0)) {
		result = do_file_op(file);
		/* expected result = OP_READ */
		report(3, OP_READ, result);
		seteuid((uid_t) 0);
...
int main(int argc, char *argv[])
{
	...
	char str[256] = "/usr/bin/nfs4_setfacl -S ";
	strcat(str, argv[3]);
	strcat(str, " ");
	strcat(str, argv[1]);
	printf("command: %s\n", str);
